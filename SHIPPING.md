...
- Debug runtime tested on Android 12 / API 31.
...
- `npm test`, `npm audit`, production TypeScript/Vite build, Capacitor sync, Android lint, APK build, and AAB build pass.
...
Gradle reads those Android values directly from the private file when building:
...
3. Run `wrangler d1 execute discipline-db --file=server/schema.sql --remote`.
...
5. Later for iOS, add `APPLE_ISSUER_ID`, `APPLE_KEY_ID`, `APPLE_PRIVATE_KEY`, and `APPLE_BUNDLE_ID` as Worker secrets.
6. Deploy with `wrangler deploy`, then use its HTTPS URL as `VITE_API_URL`.
...
6. Run `npm run release:check`. Do not upload until it passes.
7. Build, sign with the existing Discipline upload key, verify with `jarsigner -verify`, then upload the AAB to Internal testing first. Do not promote to production until login, purchase, restore, ad reward, and leaderboard tests pass from that Play-installed build.
