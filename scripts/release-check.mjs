...
import { selectReleaseFailures } from './release-check-policy.mjs';
...
async function requirePng(failures, relativePath, width, height, label, maxBytes = Infinity) {
  ...
  try {
    ...
    if (metadata.format !== 'png' || metadata.width !== width || metadata.height !== height)
      failures.push(`${relativePath}: ${label} must be PNG ${width}x${height}; found ${metadata.format || 'unknown'} ${metadata.width || '?'}x${metadata.height || '?'}`);
    if (fs.statSync(file).size > maxBytes)
      failures.push(`${relativePath}: ${label} exceeds ${Math.floor(maxBytes / 1024)} KiB`);
  } catch (error) {
    failures.push(`${relativePath}: unreadable ${label} (${error.message})`);
  }
}

await requirePng(runtimeFailures, 'public/icon-512.png', 512, 512, 'Play Store icon', 1024 * 1024);
if (!bundleOnly) {
  await requirePng(listingFailures, 'store-assets/feature-graphic.png', 1024, 500, 'Play feature graphic');
  if (!fs.existsSync(path.join(root, 'store-assets', 'feature-graphic.png'))
    ...
    listingFailures.push('store-assets/feature-graphic-source.png is rejected outdated car art and must not be submitted');
  }
  ...
  if (screenshots.length < 4) {
    listingFailures.push(`store-assets/screenshots: need the planned 4 production screenshots; found ${screenshots.length}`);
  } else {
    for (const name of screenshots) {
      ...
      try {
        ...
        if (metadata.width !== 1080 || metadata.height !== 1920)
          listingFailures.push(`store-assets/screenshots/${name}: expected 1080x1920; found ${metadata.width || '?'}x${metadata.height || '?'}`);
      } catch (error) {
        listingFailures.push(`store-assets/screenshots/${name}: unreadable screenshot (${error.message})`);
      }
    }
  }
}
...
if (!workerSource.includes("url.pathname === '/privacy'") || !legalSource.includes('privacyPage'))
  runtimeFailures.push('public privacy-policy route is missing');
...
if (/registerPlugin\s*\(\s*CapacitorGameConnectPlugin/.test(mainActivity))
  runtimeFailures.push('MainActivity must not manually register the auto-discovered Play Games plugin');
if (!/SplashScreen\.installSplashScreen\(this\)/.test(mainActivity))
  runtimeFailures.push('MainActivity is missing AndroidX legacy splash installation');
...
for (const requiredSplashEntry of ['windowSplashScreenBackground', 'windowSplashScreenAnimatedIcon', 'postSplashScreenTheme']) {
  if (!splashStyle.includes(requiredSplashEntry))
    runtimeFailures.push(`Android splash theme is missing ${requiredSplashEntry}`);
}
if (!splashStyle.includes('@mipmap/ic_launcher'))
  runtimeFailures.push('Android splash must use the approved launcher icon');
...
if (wrapperProperties.distributionSha256Sum !== 'ed1a8d686605fd7c23bdf62c7fc7add1c5b23b2bbc3721e661934ef4a4911d7c')
  runtimeFailures.push('Gradle 8.14.3 distribution SHA-256 is missing or does not match Gradle’s official checksum');
...
const failures = selectReleaseFailures({
  bundleOnly,
  listingFailures,
  runtimeFailures,
  productionFailures,
