...
<li><b>Friend referrals:</b> when a player installs through a referral link, Google Play provides the referral code, referral-click time, install time, and installed app version. The account service connects that evidence to the new account solely to validate one referral, prevent self-referrals or duplicate claims, count up to ten rewarded referrals, and grant the referrer one random unowned shop item for each rewarded referral.</li>
<li><b>Friends and multiplayer:</b> the account service stores a random public friend code, friend requests and accepted friendships, PvP invitations, selected mode and duration, server-controlled round timing, submitted tap totals or Quick Draw reaction times, match results, and the winner's verified five-Mentality reward. Platform gaming accounts are not required for this friend graph, so a DISCIPLINE. friendship can work across supported platforms.</li>
</ul></section>
<section><h2>How information is used</h2><p>Information is used to authenticate accounts, synchronize progress across devices, restore verified purchases, operate leaderboards, validate friend referrals, connect requested friends, run and settle PvP matches, grant verified winner rewards, review reports, enforce moderation decisions, prevent fraud and duplicate grants, provide rewarded and interstitial ads, diagnose failures, and maintain the game.</p></section>
<section><h2>Public leaderboard, friends, and community safety</h2><p>Your chosen username and all-time tap score are visible to other players on the public leaderboard after you accept the current Terms. Your username and friend code are also visible to players with whom you exchange a friend request, and your PvP opponent sees the mode, round state, score or reaction result, and outcome of your shared match. Other signed-in players can report a username or score and can hide an account from their own leaderboard. Reports, their selected reason, and any optional explanation are stored for operator review. Hiding is private and is not shown to the hidden player.</p></section>
<section><h2>Sharing and service providers</h2><p>Account and gameplay data is processed by the DISCIPLINE. account service and its infrastructure provider. Purchase verification and Android install-referral attribution use Google Play; purchase verification may also use Apple. Rewarded and interstitial advertising uses Google Mobile Ads. Public leaderboard viewers receive public usernames, tap totals, and ranks. Advertising and platform providers process data under their own terms and available privacy controls.</p></section>
...
<section><h2>Retention</h2><p>Primary DISCIPLINE. account data remains while the account exists. Account deletion removes the username, Terms/profile state, cloud save, leaderboard score, referral code, claims and shop-item referral rewards involving the account, friend code, friendships, PvP matches and round submissions involving the account, that account's PvP reward ledger, reports involving the account, private hide relationships, sessions, rewarded-ad ledger, and DISCIPLINE. purchase ledger from the primary account database. A reward already won by another player is retained in that winner's account ledger without retaining the deleted opponent's account record.</p>
...
<section id="deletion"><h2>Your choices and deletion</h2><p>You can revisit advertising privacy choices from Settings. A signed-in player can choose <b>Settings &gt; Delete account</b>. You may also use the <a href="/account-deletion">web account-deletion page</a> if the app is no longer installed. Account deletion also removes that account's referral code, referral claims, referral reward records, friend code, friendships, PvP matches and round submissions, and its own PvP reward ledger from the primary database. It cannot remotely erase a local save that remains on another installed device; clear the app's local storage or remove the app on that device to remove its local copy. Platform transaction, install-referral, or advertising records controlled by Google or Apple are not deleted by deleting a DISCIPLINE. account.</p></section>
...
<section><h2>Friends and PvP</h2><p>Friend requests and PvP challenges must not be used to harass or impersonate another player. PvP rounds use server timing, rate bounds, and one-time reward records. Do not automate taps, modify requests, exploit timing, coordinate false results, or otherwise manipulate a match or its five-Mentality winner reward. Exact ties continue through the displayed tiebreaker mode until the server records a winner.</p></section>
...
<section><h2 class="danger">Permanent deletion</h2><p>Deletion erases the account username, Terms/profile state, cloud progress, leaderboard score, referral code, claims and shop-item referral rewards involving the account, friend code, friendships, PvP matches and round submissions involving the account, that account's PvP reward ledger, reports involving the account, private hide relationships, inventory, sessions, rewarded-ad ledger, and DISCIPLINE. purchase ledger from the primary account database. A reward already won by another player remains in that winner's account ledger without retaining the deleted opponent's account record. It cannot remotely erase a local save that remains on another installed device; clear that device's app storage or remove the app to remove its local copy. Google Play or Apple may retain their own transaction or install-referral records.</p>
