...
<li><b>Abuse-prevention metadata:</b> for account registration and referral claims, the account service receives the network source IP supplied by Cloudflare and immediately converts it, and the relevant account/referral scope, into separate keyed HMAC fingerprints. D1 stores only the scope, pseudonymous fingerprint, fixed-window request count, and window start; it does not store the raw IP or raw referral code in this throttle table. These rows limit automated account creation and protect Google verification quota without globally limiting a public invite code.</li>
<li><b>Game and app activity:</b> cloud-save progress, settings stored in the save, inventory, purchases granted to the account, and a client-submitted cumulative tap total used for leaderboards. The account service accepts increases only within a server-side plausibility ceiling of 25 taps per second plus a small synchronization allowance; it does not independently verify each physical tap.</li>
...
<li><b>Friend referrals:</b> on Android, the app reads the referral code, referral-click time, install time, and installed app version supplied through Google Play Install Referrer. It also requests a Google Play Integrity token cryptographically bound to that exact referral claim. The account service verifies the token with Google before accepting the claim, then uses the evidence solely to prevent self-referrals or duplicate claims, count up to ten rewarded referrals, and grant the referrer one random unowned shop item for each rewarded referral. The social account and reward system is platform-neutral; an iOS claim remains disabled until a separately reviewed, server-verifiable Apple proof adapter is added.</li>
<li><b>Friends and multiplayer:</b> the account service stores a random public friend code, friend requests and accepted friendships, PvP invitations, selected mode and duration, server-controlled round timing, client-submitted and server-recorded plausibility-limited tap totals (not verified physical taps), Quick Draw raw server-receipt reaction times, bounded RTT telemetry and submission transport, match results, and eligible five-Mentality winner rewards. At most ten wins per winner and one result per unordered player pair can be rewarded in a rolling 24-hour window, so a completed win may award zero. Platform gaming accounts are not required for this friend graph, so a DISCIPLINE. friendship can work across supported platforms.</li>
</ul></section>
<section><h2>How information is used</h2><p>Information is used to authenticate accounts, rate-limit automated registration and referral-proof abuse, synchronize progress across devices, restore verified purchases, operate leaderboards, validate friend referrals, connect requested friends, run and settle PvP matches, grant eligible winner rewards, review reports, enforce moderation decisions, prevent fraud and duplicate grants, provide rewarded and interstitial ads, diagnose failures, and maintain the game.</p></section>
<section><h2>Public leaderboard, friends, and community safety</h2><p>Your chosen username and client-submitted cumulative tap total accepted under the server-side plausibility ceiling are visible to other players on the public leaderboard after you accept the current Terms. This score is not an independently verified count of physical touches. Your username and friend code are also visible to players with whom you exchange a friend request, and your PvP opponent sees the mode, round state, score or reaction result, and outcome of your shared match. Other signed-in players can report a username or score and can hide an account from their own leaderboard. Reports, their selected reason, and any optional explanation are stored for operator review. Hiding is private and is not shown to the hidden player.</p></section>
<section><h2>Sharing and service providers</h2><p>Account and gameplay data is processed by the DISCIPLINE. account service and its infrastructure provider. Purchase verification and Android install-referral attribution use Google Play; purchase verification may also use Apple. Rewarded and interstitial advertising uses Google Mobile Ads. Public leaderboard viewers receive public usernames, client-submitted plausibility-limited cumulative tap totals, and ranks. Advertising and platform providers process data under their own terms and available privacy controls.</p></section>
...
<section><h2>Retention</h2><p>Primary DISCIPLINE. account data remains while the account exists. Account deletion removes the username, Terms/profile state, cloud save, leaderboard score, referral code and personal referral attribution involving the deleted account, friend code, friendships, PvP matches and round submissions involving the account, that account's PvP reward ledger, reports involving the account, private hide relationships, sessions, rewarded-ad ledger, and DISCIPLINE. purchase ledger from the primary account database. A keyed HMAC fingerprint of one-use install evidence remains to prevent the same evidence being claimed by another account; it does not retain the referral code or install timestamps. A referral reward already earned by another player also remains in that player's lifetime reward ledger, without retaining the deleted referred account's identifier. Pseudonymous rate-limit fingerprints and their counts are pruned by an hourly task once their window start is more than 24 hours old, so normal retention is less than 25 hours; they are not a permanent account record.</p>
...
<section id="deletion"><h2>Your choices and deletion</h2><p>You can revisit advertising privacy choices from Settings. A signed-in player can choose <b>Settings &gt; Delete account</b>. You may also use the <a href="/account-deletion">web account-deletion page</a> if the app is no longer installed. Account deletion removes that account's referral code and personal referral attribution, friend code, friendships, PvP matches and round submissions, and its own PvP and referral reward ledgers from the primary database. It does not revoke a referral reward already earned by another player: a keyed HMAC fingerprint of the one-use evidence and that other player's reward/count entry remain, with the referral code, install timestamps, and deleted referred account identifier erased. It cannot remotely erase a local save that remains on another installed device; clear the app's local storage or remove the app on that device to remove its local copy. Platform transaction, install-referral, or advertising records controlled by Google or Apple are not deleted by deleting a DISCIPLINE. account.</p></section>
...
<p>These Terms govern use of the DISCIPLINE. game account, cloud save, public leaderboard, friends, and multiplayer.</p>
...
<section><h2>Friends and PvP</h2><p>Friend requests and PvP challenges must not be used to harass or impersonate another player. PvP rounds use server timing, plausibility bounds, and one-time reward records. There is no Mentality wager. An eligible win grants five Mentality, limited to ten rewarded wins per winner and one rewarded result per unordered player pair in any 24-hour window. A completed win can therefore grant zero. Both players must have participated during the match for a forfeit win to be reward-eligible. Do not automate taps, modify requests, exploit timing, coordinate false results, or otherwise manipulate a match or its reward. An exact tie starts one opposite-mode tiebreaker; if that tiebreaker also ties, the match ends without a winner or reward. After a round deadline, one participant and one no-show produces a forfeit win; no input from either player remains a no-contest. Removing a friend, blocking an opponent, or deleting an account during an active match forfeits to the opponent, while an invitation that has not begun is cancelled.</p></section>
...
<section><h2 class="danger">Permanent deletion</h2><p>Deletion erases the account username, Terms/profile state, cloud progress, leaderboard score, referral code and personal referral attribution, friend code, friendships, PvP matches and round submissions involving the account, that account's PvP and referral reward ledgers, reports involving the account, private hide relationships, inventory, sessions, rewarded-ad ledger, and DISCIPLINE. purchase ledger from the primary account database. A referral reward already earned by another player remains in that player's lifetime ledger; only a keyed HMAC fingerprint of the one-use evidence and the reward/count entry remain, while the referral code, install timestamps, and deleted referred account identifier are erased. Pseudonymous anti-abuse rate-limit fingerprints are not permanent account records and are removed by the hourly cleanup after their window start is more than 24 hours old (normally in less than 25 hours). A PvP reward already won by another player likewise remains without retaining the deleted opponent's account record. It cannot remotely erase a local save that remains on another installed device; clear that device's app storage or remove the app to remove its local copy. Google Play or Apple may retain their own transaction or install-referral records.</p>
...
const deleteCloudData=async token=>{let prior='';for(let page=0;page<128;page++){const removed=await fetch('/v1/account',{method:'DELETE',headers:{Authorization:'Bearer '+token}});const data=await removed.json().catch(()=>({}));if(!removed.ok)throw new Error('Deletion failed. Please try again.');if(data.deleted===true)return;if(removed.status!==202||data.deletionPending!==true)throw new Error('Deletion failed. Please try again.');const progress=String(data.progressToken||'');if(!progress||progress===prior)throw new Error('Deletion paused safely. Submit again to continue.');prior=progress;const requested=Number(data.retryAfterMs);await wait(Number.isFinite(requested)?Math.max(100,Math.min(1000,Math.trunc(requested))):100)}throw new Error('Deletion is still processing. Submit again to continue.')};
form.addEventListener('submit',async(event)=>{event.preventDefault();if(confirmInput.value.trim().toUpperCase()!=='DELETE')return;button.disabled=true;status.textContent='Verifying account...';try{const login=await fetch('/v1/auth/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:document.getElementById('username').value.trim(),password:document.getElementById('password').value})});const auth=await login.json();if(!login.ok)throw new Error('Username or password is incorrect.');status.textContent='Deleting account and associated data...';await deleteCloudData(auth.token);form.remove();status.textContent='Account deleted. Your DISCIPLINE. cloud data has been erased.'}catch(error){status.textContent=error.message||'Deletion failed. Please try again.';button.disabled=false}});
</script>`);
};
