...
<section><h2>How information is used</h2><p>Information is used to authenticate accounts, synchronize progress across devices, restore verified purchases, operate leaderboards, validate friend referrals, review reports, enforce moderation decisions, prevent fraud and duplicate grants, provide rewarded and interstitial ads, diagnose failures, and maintain the game.</p></section>
...
<section id="deletion"><h2>Your choices and deletion</h2><p>You can revisit advertising privacy choices from Settings. A signed-in player can choose <b>Settings &gt; Delete account</b>. You may also use the <a href="/account-deletion">web account-deletion page</a> if the app is no longer installed. Account deletion also removes that account's referral code, referral claims, and referral reward records from the primary database. It cannot remotely erase a local save that remains on another installed device; clear the app's local storage or remove the app on that device to remove its local copy. Platform transaction, install-referral, or advertising records controlled by Google or Apple are not deleted by deleting a DISCIPLINE. account.</p></section>
